/**  
 * @file csv.c
 * @author Steven Dahdah, Antoine Henri
 * @brief Simple library for writing CSV files containing only floating point numbers
 * and a header.
 * @version 1.1
 * @date 2024-02-27
 * 
...
/** count the number of columns in a csv file based on the header string
 *
 * Must only be called in csv_init
 *
 * @param[in] header Header string, with each column name seperated with ", "
 * (comma, then space).
 *
 * @retval Number of columns in the csv file
 */
unsigned int csv_header_count(char* header);
...
/** Prints csv error code value
 *
 * @param[in] csv_status Error code to print
 */
void csv_print_error(CsvStatus csv_status);
